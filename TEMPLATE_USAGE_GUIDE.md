...
result = provider.generate_analysis(
    prompt="请分析这个内容",
    content_info={"type": "video", "title": "示例视频"}
)
...
class CachedAIProvider:
    ...
    def generate_analysis(self, prompt, content_info):
        # 生成缓存键
        cache_key = f"analysis:{hash(prompt)}:{content_info.get('type')}"
        ...
        # 生成新结果并缓存
        result = self.provider.generate_analysis(prompt, content_info)
        if result.success:
...
@celery_app.task
def analyze_content_async(content_id, prompt):
    provider = LLMProviderFactory.create_default_provider()
    result = provider.generate_analysis(prompt, {"id": content_id})
